...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("**IcssBot") and event.query.user_id == bot.uid:
            ...
            buttons = [
                custom.Button.inline("اظهار الرساله 🍄.", data=f"secret_{timestamp}")
            ]
            result = builder.article(
                title=" ارسال الهمسه 💕.",
                text=f"**⌔∮ هذه الهمسه من جافا الي {Kimo} هو الوحيد الذي يستطيع رؤيتها.**",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid: 
            ...
            reply_pop_up_alert = "⌔∮ عليك تنصيب السورس اولا للحصول عليه اذهب الى @JAI6H"
